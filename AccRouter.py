...
class AccRouter():
    ...
    async def okx(self, okx_starknet_address=self.wallet.okx_starknet_address):
        if not okx_starknet_address:
            return -1, 'No okx address for this wallet'
        status, txn = await sender_hand.create_txn(eth, self.wallet.okx_starknet_address, self.account)
        if status < 0:
            return status, txn
