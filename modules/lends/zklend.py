...
from modules.utils.utils import req, handle_dangerous_request
...
class ZkLend(BaseLend):
    ...
    lend_tokens = [
        StarkToken("ETH-SUPPLIED", 0x01b5bd713e72fdc5d63ffd83762f81297f6175a5e0a4771cdadbc1dd5fe72cb1, 18),
        StarkToken("WBTC-SUPPLIED", 0x02b9ea3acdb23da566cee8e8beae3125a1458e720dea68c4a9a7a2d8eb5bbb4a, 8),
        StarkToken("USDT-SUPPLIED", 0x00811d8da5dc8a2206ea7fd0b28627c2d77280a515126e62baa4d78e22714c4a, 6),
        StarkToken("USDC-SUPPLIED", 0x047ad51726d891f972e74e4ad858a261b43869f7126ce7436ee0b2529a98f486, 6),
        StarkToken("DAI-SUPPLIED", 0x062fa7afe1ca2992f8d8015385a279f49fad36299754fb1e9866f4f052289376, 18)
    ]
    ...
    async def get_prices(self, sender: BaseAccount):
        while True:
            try:
                r = await req("https://data.app.zklend.com/pools")
                break
            except Exception as e:
                logger.error(f"[{sender.stark_address}] can't get zklend prices: {e} trying again")
        res = {}
        for info in r:
            res[info["token"]["symbol"].upper()] = int(info["price"]["price"], 16)/1e8
        return res

    async def get_total_borrowed(self, sender: BaseAccount):
        ...
        for token_name in self.supported_tokens:
            ...
            val_in_token_wei = (await handle_dangerous_request(
                contract.functions["get_user_debt_for_token"].call, 
                f"can't get borrowed {token_name}", 
    ...
    async def get_total_supplied(self, sender: BaseAccount):
        prices = await self.get_prices(sender)
        val = 0
        ...
        for token_name in self.supported_tokens:
            ...
            lend_token_val_wei = await handle_dangerous_request(
                sender.stark_native_account.get_balance, 
                f"can't get balance of {lend_token.symbol} Error:", 
                sender.stark_address, 
                lend_token.contract_address
            )
    ...
    async def create_txn_for_adding_token(self, token: StarkToken, amount: float, sender: BaseAccount):
        contract = Contract(self.contract_address, self.ABI, sender.stark_native_account)
        ...
        coll_enabled = (await handle_dangerous_request(
            contract.functions["is_collateral_enabled"].call, 
            "can't check for collateral Error", 
            sender.stark_address, 
            sender.stark_native_account.address, 
            token.contract_address
        )).enabled
    ...
    async def create_txn_for_removing_token(self, amount: int, token: StarkToken, sender: BaseAccount):
        prices = await self.get_prices(sender)
        contract = Contract(self.contract_address, self.ABI, sender.stark_native_account)
        stark_token: StarkToken = self.token_from_name[token.symbol.split("-")[0]]
        ...
        token_bal = (await sender.get_balance_starknet(token))[0] - 1
    ...
    async def create_txn_for_return(self, token: StarkToken, sender: BaseAccount):
        contract = Contract(self.contract_address, self.ABI, sender.stark_native_account)
        val_in_token_wei = (await handle_dangerous_request(
            contract.functions["get_user_debt_for_token"].call, 
            f"can't get borrowed {token.symbol}", 
            sender.stark_address, 
            sender.stark_native_account.address, 
            token.contract_address
        )).debt
        
        bal = (await sender.get_balance_starknet(token))[0]
