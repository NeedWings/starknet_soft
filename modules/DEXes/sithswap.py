...
class SithSwap(BaseDEX):
    ...
    supported_tokens = ["ETH", "USDT", "USDC", "DAI", "WBTC", "LORDS", "WSTETH", "SHIT_COIN"]
    
    lpts = [
        StarkToken("ETH:USDT", 0x691fa7f66d63dc8c89ff4e77732fff5133f282e7dbd41813273692cc595516, 18),
        StarkToken("ETH:USDC", 0x30615bec9c1506bfac97d9dbd3c546307987d467a7f95d5533c2e861eb81f3f, 18),
        StarkToken("ETH:DAI", 0x32ebb8e68553620b97b308684babf606d9556d5c0a652450c32e85f40d000d, 18),
        StarkToken("ETH:WBTC", 0x106ff0f48fba1274b1f4f65e6d847fa58ac455e95318754fec20eb221b660ed, 18),
        StarkToken("ETH:WSTETH", 0xa20410b97dcbfb44f370b67f877c669d859ff85c536d5e8e8336b6d565e1f8, 18),
        StarkToken("ETH:LORDS", 0x2718bbc0176cff083b5c95ac22691bd1c16d55d37ae153d3ba347ce2292417e, 18),
        StarkToken("USDT:USDC", 0x601f72228f73704e827de5bcd8dadaad52c652bb1e42bf492d90bbe22df2cec, 18),
        StarkToken("USDT:DAI", 0x7500d8ba71b0a0c587fa431753dbb01a4595a434f27b296358c2c4cb7f83585, 18),
        StarkToken("USDT:LORDS", 0x351d125294ae90c5ac53405ebc491d5d910e4f903cdc5d8c0d342dfa71fd0e9, 18),
        StarkToken("USDC:DAI", 0x15e9cd2d4d6b4bb9f1124688b1e6bc19b4ff877a01011d28c25c9ee918e83e5, 18),
        StarkToken("USDC:WBTC", 0x24bd1600bbe18593e983f858f2e7a69148e6b973f43b6304b2b8cf110059595, 18),
        StarkToken("USDC:WSTETH", 0x1f410986ac70a3d2f945bedc47133abe0d20d95e0cb65cd16e9e77e03bcd9d9, 18),
        StarkToken("USDC:LORDS", 0x1283d9f872498556e11b48ec440d66ebe55c6624bc34b737ffb3939fc08203e, 18),
        StarkToken("DAI:WBTC", 0x68fa77586bc98d71125e342660ebdfb17a1c26027002af1dd8a620d93ea3ba3, 18),
        StarkToken("DAI:LORDS", 0x1eb8805c2f3cc4a7ac8ddde3f8b3e1d7d197053fa6050e318de2100e4eb2d18, 18)
    ]
    ...
    async def get_token1_for_token2_price(self, token1: StarkToken, token2: StarkToken, amount_in: float, sender: BaseAccount):
        ...
        amount_out = (await handle_dangerous_request(
            contract.functions["getAmountOut"].call,
            "can't get pool info",
        ...
        amount_out = amount_out/10**token2.decimals
    ...
    async def create_txn_for_swap(self, amount_in: float, token1: StarkToken, amount_out: float, token2: StarkToken, sender: BaseAccount, full: bool = False):
        ...
        exists = await handle_dangerous_request(
            self.check_existance, 
            "can't get pool info on sithswap", 
            sender.stark_address, 
            token1.contract_address, 
            token2.contract_address, 
            stable, 
        ...
        if not exists:
            logger.error(f"[{sender.stark_address} got unsupported pool on sithswap. Skip")
            return -1
    ...
    async def create_txn_for_remove_liq(self, lptoken: StarkToken, sender: BaseAccount):
        ...
        total_liq_amount = (await handle_dangerous_request(
            ...
            "Can't get sithswap total pool value. Error:", 
            sender.stark_address
        )).totalSupply
